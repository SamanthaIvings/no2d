...
def load_survey(xlsx: Path | str | None = None) -> pd.DataFrame:
    """Load the FF survey responses. Defaults to data/inputs/ (plain inputs,
    not the fw_inputs_sy_simplified subfolder)."""
    if xlsx is None:
        xlsx = fc.data_path("inputs", "FF_Survey_responses.xlsx")
        if not Path(xlsx).exists():
            xlsx = "FF_Survey_responses.xlsx"  # cwd fallback
    return pd.read_excel(xlsx, sheet_name="Responses")


def build_figure(df: pd.DataFrame):
    fig, axes = plt.subplots(2, 3, figsize=(15, 9))
    fig.suptitle("FF Survey — Regression vs ±1 SD Functional Form Comparison",
...
def main(out_dir: Path | str | None = None, xlsx: Path | str | None = None) -> None:
    mpl.rcParams.update({
    ...
    if out_dir is None:
        out_dir = fc.data_path("plots", "sd_simulations")
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    out_png = out_dir / "ff_survey_sd_comparison.png"
